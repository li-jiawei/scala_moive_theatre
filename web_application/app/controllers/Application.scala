...
class Application @Inject() (implicit system: ActorSystem, materializer: Materializer, conf: Configuration) extends Controller {
  ...
  // A uri to stop spark streaming
  def stop() = Action {
    adminActor ! SparkStreaming(false, conf)
    Ok("goood")
  }

  //  Accept a websocket connection with new API
  def requestStreaming = WebSocket.accept[JsValue, JsValue]{ implicit request =>
    ActorFlow.actorRef(out => WebsocketActor.props(out))
  }

  // just a try, Accept a websocket connection and process data using iteratee and enuerator with old api

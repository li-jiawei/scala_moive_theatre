...
class AdminActor extends Actor with ActorLogging {
  ...
  override def receive = {
    case TransferDao(dao) =>
      ...
      movieDao = dao
      consumerActor = context.actorOf(RoundRobinPool(5, Some(resizer), supervisorStrategy).props(ConsumerActor.props(movieDao)), "router2")//context.actorOf(ConsumerActor.props(movieDao), name = "consumerActor")
